...
def createSetOfFunctionSymbolsAtXSet(
    xSet, functionSymbolStr=_DefaultFunctionSymbolStr, sameSubscriptsAsStencil=False
):
...
def dotproduct_no_eval(numr, fSet):
    ...
    for i in range(idx_start - 1, idx_end, -1):
        eq = sp.Add(eq, sp.Mul(numr[i], fSet[i], evaluate=False), evaluate=False)
